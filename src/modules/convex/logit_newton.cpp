...
typedef Newton<NewtonState<MutableArrayHandle<double> >, NewtonState<ArrayHandle<double> >,
        Logit<GLMModel, GLMTuple, GLMHessian > > LogitNewtonAlgorithm;

typedef Loss<NewtonState<MutableArrayHandle<double> >, NewtonState<ArrayHandle<double> >,
...
logit_newton_transition::run(AnyType &args) {
    ...
    NewtonState<MutableArrayHandle<double> > state = args[0];

    // initilize the state if first tuple
    if (state.algo.numRows == 0) {
        if (!args[3].isNull()) {
            NewtonState<ArrayHandle<double> > previousState = args[3];
...
logit_newton_merge::run(AnyType &args) {
    NewtonState<MutableArrayHandle<double> > stateLeft = args[0];
    NewtonState<ArrayHandle<double> > stateRight = args[1];
...
internal_logit_newton_distance::run(AnyType &args) {
    NewtonState<ArrayHandle<double> > stateLeft = args[0];
    NewtonState<ArrayHandle<double> > stateRight = args[1];
...
internal_logit_newton_result::run(AnyType &args) {
    NewtonState<ArrayHandle<double> > state = args[0];
