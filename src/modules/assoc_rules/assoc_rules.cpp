...
namespace madlib
{
  namespace modules
  {
	namespace ar
	{
		 ...
		 *
		 */
        void* gen_rules_from_cfp::SRF_init(AnyType &args)
        {
			perm_fctx			*myfctx		= NULL;
			char				*positions  = args[0].getAs<char*>();

			/* allocate memory for user context */
			myfctx = (perm_fctx *) palloc(sizeof(perm_fctx));
			...
			myfctx->flags 		= (bool*) palloc0(sizeof(bool) *
											myfctx->num_elems);
		 ...
		 *
		 */
        AnyType gen_rules_from_cfp::SRF_next(void *user_fctx, bool *is_last_call)
        {
        	...
        	if (!is_last_call)
        		elog(ERROR, "the paramter is_last_class should not be null");
			...
			pre_text 	= (char*) palloc0(sizeof(char) * myfctx->pos_len);
			post_text 	= (char*) palloc0(sizeof(char) * myfctx->pos_len);
			result		= (Datum*) palloc0(sizeof(Datum) << 1);
			p_pre		= pre_text;
			p_post		= post_text;
