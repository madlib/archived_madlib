...
/**
 * @brief Transition state for the robust variance calculations of the 
 ...
 */
template <class Handle>
class RobustCoxPropHazardsTransitionState {

    template <class OtherHandle>
    friend class RobustCoxPropHazardsTransitionState;

public:
    RobustCoxPropHazardsTransitionState(const AnyType &inArray)
      : mStorage(inArray.getAs<Handle>()) {
     ...
     */
    template <class OtherHandle>
    RobustCoxPropHazardsTransitionState &operator=(
        const RobustCoxPropHazardsTransitionState<OtherHandle> &inOtherState) {
...
private:
    static inline size_t arraySize(const uint16_t inWidthOfX) {
        return 5 + 3*inWidthOfX + 3*inWidthOfX*inWidthOfX;
    }
     ...
     * - 0: numRows (number of rows seen so far)
     * - 1: widthOfX (number of features)
     * - 2: multiplier (resolving tied time of deaths)
     * - 3: y_previous (resolving tied time of deaths)
     * - 4: coef (multipliers for each of the features)
		 
		 * Intra interation components (updated in the current interation)
     * - 4 + widthofX: S (see design document for details)
     * - 5 + widthofX: Hi[j] (see design document for details)
     * - 5 + 2*widthofX: grad (gradient)
     * - 5 + 3*widthofX: V (Precomputations for the hessian)
     * - 5 + 3*widthofX + widthofX^2: hessian
     * - 5 + 3*widthofX + 2*widthofX^2: hessian
     *
     */
    void rebind(uint16_t inWidthOfX) {
        ...
        x_tied_sum.rebind(&mStorage[5+2*inWidthOfX], inWidthOfX);
				...
				hessian.rebind(&mStorage[5+3*inWidthOfX+inWidthOfX*inWidthOfX],
					inWidthOfX, inWidthOfX);
				grad_grad_trans.rebind(&mStorage[5+3*inWidthOfX+2*inWidthOfX*inWidthOfX],
					inWidthOfX, inWidthOfX);
...
AnyType robust_cox_prop_hazards_step_transition::run(AnyType &args) {
 ...
 *
 */
AnyType robust_cox_prop_hazards_step_final::run(AnyType &args) {
    ...
    MutableMappedColumnVector robust_std_err(
        allocateArray<double>(state.coef.size()));
    MutableMappedColumnVector robustZStats(
        allocateArray<double>(state.coef.size()));
    MutableMappedColumnVector robustPValues(
        allocateArray<double>(state.coef.size()));

    for (Index i = 0; i < state.coef.size(); ++i) {
        robust_std_err(i) = std::sqrt(diagonal_of_varCovar(i));
        robustZStats(i) = state.coef(i) / robust_std_err(i);
        robustPValues(i) = 2. * prob::cdf( prob::normal(),
            -std::abs(robustZStats(i)));
		}
		
    // Return all coefficients, standard errors, etc. in a tuple
    AnyType tuple;
    tuple << robust_std_err << robustZStats << robustPValues;
