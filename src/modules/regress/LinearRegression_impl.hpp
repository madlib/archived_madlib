...
RobustLinearRegressionAccumulator<Container>::operator<<(const tuple_type& inTuple) {
    ...
    const MappedColumnVector& coef = std::get<2>(inTuple);
		...
		// Checks for finiteness and maximum number of variables are not performed
		// here because they are already performed in regular linear regression.
		// Note: Robust linear regression is called after regular linear regression
