...
(
    ...
    ORDER BY 1, 2, 4
) q """.format(schema_name=schema_name))

$$ LANGUAGE plpythonu;


DROP TABLE IF EXISTS functions_madlib;
DROP TABLE IF EXISTS functions_madlib_v141;

SELECT get_functions('madlib');
...
(
SELECT
    v141.name, v141.retype, v141.argtypes
FROM
    functions_madlib_v141 AS v141
    LEFT JOIN
    functions_madlib AS v15
    USING (name, retype, argtypes)
WHERE v15.name IS NULL
ORDER by v141.type, v141.name
) q;
