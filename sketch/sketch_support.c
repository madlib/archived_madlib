...
unsigned int rightmost_one(unsigned char *bits, size_t numsketches, size_t sketchsz_bits, size_t sketchnum)
{
  ...
  if (sketchsz_bits % (sizeof(unsigned int)*CHAR_BIT))
    elog(ERROR, "number of bits per sketch is %lu, must be a multiple of sizeof(unsigned int) = %lu",
         sketchsz_bits, (unsigned long)sizeof(unsigned int));
...
unsigned int leftmost_zero(unsigned char *bits, size_t numsketches, size_t sketchsz_bits, size_t sketchnum)
{
  ...
  if (sketchsz_bits > numsketches*8)
    elog(ERROR, "sketch sz declared at %lu, but bitmap is only %lu", (unsigned long)sketchsz_bits, (unsigned long)numsketches*8);
  ...
  {         
    ...
    else
    {
      ...
      unsigned char b = ((s[i] * 0x0802LU & 0x22110LU) | (s[i] * 0x8020LU & 0x88440LU)) * 0x10101LU >> 16; 
                             ...
                             int4 sketchsz_bits, int4 sketchnum, int4 bitnum)
{
  ...
  if (sketchnum >= numsketches)
    elog(ERROR, "sketch offset exceeds the number of sketches (0-based)");
  if (bitnum >= sketchsz_bits)
    elog(ERROR, "bit offset exceeds the number of bits per sketch (0-based)");
  if (sketchsz_bits % sizeof(unsigned int))
    elog(ERROR, "number of bits per sketch is %d, must be a multiple of sizeof(unsigned int) = %lu",
         sketchsz_bits, (unsigned long)sizeof(unsigned int));

  mask = 0x1 << bitnum%8;
  ((char *)(VARDATA(bitmap)))[sketchnum*sketchsz_bits/CHAR_BIT + (sketchsz_bits/CHAR_BIT - 1 - bitnum/CHAR_BIT)] |= mask;
