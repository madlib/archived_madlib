...
// In addition, we are going to play the "dyadic range" trick mentioned in Cormode/Muthu,
// which repeats the above idea log_2(n) times as follows.  Every value x/(2^i) is "sketched"
...
int64 hash_counters_iterate(Datum, int64 *, int64, int64 (*lambdaptr)(unsigned int, unsigned int, int64 *, int64));
bool gt(int64, int64);
bool eq(int64, int64);
bool false_fn(int64, int64);
int64 cmsketch_count_search(cmtransval *, bool (*)(int64, int64), int64, bool (*)(int64, int64), int64);
 ...
 */
void countmin_dyadic_trans_c(int64 *counters, int64 inputi, cmtransval *transval)
{
    char *newstring;
    int   j;
...
Datum cmsketch_getcount(PG_FUNCTION_ARGS)
{
    ...
    /* get the provided element, being careful in case it's NULL */
    if (!PG_ARGISNULL(1)) {
        PG_RETURN_INT64(cmsketch_getcount_c(transval->outFuncOid, (int64 *)transval->counters, 
                        PG_GETARG_INT64(1)));
     }
    else
        PG_RETURN_NULL();
...
int64 cmsketch_getcount_c(Oid funcOid, int64 *counters, Datum arg)
{
    ...
    PG_RETURN_INT64(hash_counters_iterate(nhash, counters, LONG_MAX, &min_counter));
}
...
Datum cmsketch_histogram_c(cmtransval *transval, int64 min, int64 max, int buckets)
{
    int64      step;
    int        i;
    ArrayType *retval;
...
*/
int64 hash_counters_iterate(Datum hashval, int64 *counters, int64 initial, int64 (*lambdaptr)(unsigned int, unsigned int, int64 *, int64))
{
    unsigned int i, col;
...
int64 increment_counter(unsigned int i, unsigned int col, int64 *counters, int64 transval)
{
...
int64 min_counter(unsigned int i, unsigned int col, int64 *counters, int64 transval)
{
