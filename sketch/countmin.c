...
Datum cmsketch_trans(PG_FUNCTION_ARGS)
{
    ...
    if (VARSIZE(transblob) <= VARHDRSZ+8) {
         ...
         */
        blobsz = VARHDRSZ + sizeof(cmtransval) + numsketches*ROWS*
                 COLUMNS*sizeof(int64);
        /*
        ...
        getTypeOutputInfo(element_type,
                          &(transval->outFuncOid),
                          &typIsVarlena);
    }
    else {
    ...
    if (!PG_ARGISNULL(1)) {
        ...
        case INT8OID:
             ...
             * case FLOAT8OID:
             */
            countmin_num_trans_c(counters, PG_GETARG_DATUM(
                                     1), transval);
        }
 ...
 */
    ...
    nhash =
        DirectFunctionCall2(binary_decode, result,
...
Datum countmin_getcount_c(int64 *counters, char *input)
{
    ...
    result =
        DirectFunctionCall1(md5_bytea,
                            PointerGetDatum(cstring_to_text(input)));
...
void find_ranges(Datum bot, Datum top, int power, rangelist *r, Oid typeOid)
{
    ...
    if ((int64)DatumGetValTypeOid(bot) % raised == 0) {
        ...
        r->spans[r->next][1] =
            ValGetDatumTypeOid(DatumGetValTypeOid(bot) + raised - 1);
        /* elog(WARNING, "atbot %ld: [%ld, %ld]\n", raised, r->spans[r->next][0], r->spans[r->next][1]); */
        r->next++;
        find_ranges(ValGetDatumTypeOid(DatumGetValTypeOid(bot) +
                                       (raised)), top, i-1, r, typeOid);
    }
    else if ((int64)(DatumGetValTypeOid(top)+1) % raised == 0) {
        r->spans[r->next][0] =
            ValGetDatumTypeOid(DatumGetValTypeOid(top) - raised + 1);
        r->spans[r->next][1] = top;
        /* elog(WARNING, "attop %ld: [%ld, %ld]\n", raised, r->spans[r->next][0], r->spans[r->next][1]); */
        r->next++;
        find_ranges(bot,
                    ValGetDatumTypeOid(DatumGetValTypeOid(
                                           top) - raised), i-1, r,
                    typeOid);
    }
    else {     /* we straddle a boundary */
        find_ranges(bot,
                    ValGetDatumTypeOid(raised * (DatumGetValTypeOid(
                                                     top)/
                                                 raised)) - 1, i-1, r,
                    typeOid);
        find_ranges(ValGetDatumTypeOid(raised *
                                       (DatumGetValTypeOid(
                                            top)/raised)), top, i-1,
                    r, typeOid);
    }
...
Datum cmsketch_rangecount(PG_FUNCTION_ARGS)
{
    ...
    if (element_type1 != transval->elementTypeOid || element_type2 !=
        transval->elementTypeOid) {
        elog(
            ERROR,
            "sketch computed over %Ld type; boundaries are %Ld and %Ld.  consider casting.",
            (long long)transval->elementTypeOid,
            (long long)element_type1,
            (long long)element_type2);
        PG_RETURN_NULL();
    }

    if (translen == sizeof(cmtransval) + ROWS*COLUMNS*sizeof(int64) +
        VARHDRSZ) {
        elog(WARNING,
             "Cannot perform rangecount for a non-integer type");
        PG_RETURN_NULL();
    }

    return cmsketch_rangecount_c(transval, PG_GETARG_DATUM(
                                     1), PG_GETARG_DATUM(2));
}
...
Datum cmsketch_rangecount_c(cmtransval *transval, Datum bot, Datum top)
{
    ...
    for (i = 0; i < r.next; i++) {
        /* XXX ARITHMETIC */
        sketchno = log2(DatumGetValTypeOid(
                            r.spans[i][1]) -
                        DatumGetValTypeOid(r.spans[i][0]) + 1);
        newstring = OidOutputFunctionCall(transval->outFuncOid,
                                          r.spans[i][0] >> sketchno);
        /* get the md5 hash of the input.  Return val is textual representation of a big hex number. */
        result =
            DirectFunctionCall1(md5_bytea,
                                PointerGetDatum(cstring_to_text(
                                                    newstring)));
        /* digest = (bytea *)DatumGetPointer(result); */
        nhash = DirectFunctionCall2(binary_decode,
                                    result,
                                    CStringGetTextDatum("hex"));
...
Datum cmsketch_histogram_c(cmtransval *transval, int buckets)
{
    ...
    for (i = 0, loguess = minval, higuess = maxval;
         (i < TYPLEN(transval->elementTypeOid) - 1) && (higuess-loguess > 1);
         i++) {
        curcount = cmsketch_rangecount_c(transval, minval, curguess);
        /*
         * elog(NOTICE, "curgess: %ld, curcount: %ld, curpct: %f",
    ...
    min = (cmsketch_rangecount_c(transval, minval, curguess) == 0) ?
          (int64)DatumGetValTypeOid(curguess) : (int64)DatumGetValTypeOid(
        loguess);
...
Datum cmsketch_combine(PG_FUNCTION_ARGS)
{
    ...
    if (VARSIZE(counterblob2) == VARHDRSZ)
        ...
        for (i = 0;
             i < (VARSIZE(newblob) - VARHDRSZ)/sizeof(int64);
             i++)
        ...
        elog(
            ERROR,
            "Attempting to combine sketches of different sizes: %d and %d.",
            VARSIZE(counterblob1),
            VARSIZE(counterblob2));
        PG_RETURN_NULL();
    }
