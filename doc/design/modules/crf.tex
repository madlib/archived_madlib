...
\begin{algorithm}[merge-lbfgs$(\mathit{state1}, \mathit{state2})$] \label{alg:merge-lbfgs}
\alginput{Transition state $\mathit{state1}$,\\
Transition state $\mathit{state2}$}
\algoutput{Transition state $\mathit{new\_state}$}
\begin{algorithmic}[1]
...
\begin{algorithmic}[1]
        \State $penalty()$ \Comment{To avoid overfitting, penalize the likelihood/gradient with a Gaussian prior} 
