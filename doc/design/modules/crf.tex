...
\begin {table}
\caption {Training data} \label{tab:trainingdata} 
\begin{center}
...
\item $SQL_1$: Extracting edge features:\\
	       \fbox{%
		       \parbox{0.95\linewidth}{%
                        SELECT $doc2.start\_pos, doc2.doc\_id$, $'E.'$, $ARRAY[doc1.label, doc2.label]$\\
                        FROM   segmenttbl  $doc1$, segmenttbl $doc2$\\
                        WHERE  $doc1.doc\_id = doc2.doc\_id$ AND $doc1.start\_pos+1 = doc2.start\_pos$
		       }%
	       }\\
...
\item $SQL_2$: Extracting Regex features:\\
	       \fbox{%
		       \parbox{0.95\linewidth}{%
                        SELECT $start\_pos, doc\_id, 'R\_' || name$, $ARRAY[-1, label]$\\
                        FROM  regextbl, segmenttbl\\
                        WHERE $seg\_text \sim pattern$
		       }%
	       }\\
...
\item $SQL_1$: Extracting unique tokens:\\
	       \fbox{%
		       \parbox{0.95\linewidth}{%
                        INSERT INTO $segment\_hashtbl$\\
                        SELECT DISTINCT $seg\_text$\\
                        FROM   segmenttbl
		       }%
	       }\\
...
\item $SQL_2$: Summerize over all single state features with respect to specific tokens and labels :\\
	       \fbox{%
		       \parbox{0.95\linewidth}{%
                        INSERT INTO $viterbi\_rtbl$\\
                        SELECT $seg\_text$, label, SUM(value)\\
                        FROM   rtbl\\
                        GROUP BY $seg\_text,label$
		       }%
	       }\\
