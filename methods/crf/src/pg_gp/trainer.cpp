...
void trainer::train(FILE * fout) {
	...
	f = compute_logli_gradient(lambda, gradlogli, num_iters + 1, fout);
...
double trainer::compute_logli_gradient(double * lambda, double * gradlogli, 
...
void trainer::compute_log_Mi(sequence & seq, int pos, doublematrix * Mi, 
