...
/*
 * The functionality of this function is the same with palloc0, except that
 * we explicitly set the size to (1 * unit_size) if it's 0, and check
 * if the memory allocation is failed or not.
 *
 * Parameters:
 *      num_units	The number of memory units should be allocated
 *      unit_size   	The number of bytes per each memory unit
 *
 * Return:
 *      The pointer points to the allocated memory which size is
 *      (num_units * unit_size)
 */
void*
dt_palloc0
	...
	)
{
	...
	void *ptr = palloc0((num_units) * (unit_size));
	dt_check_error(ptr, "memory allocation failure");
...
/*
 * The functionality of this function is the same with palloc, except that
 * we explicitly set the size to (1 * unit_size) if it's 0,
 * and check if the memory allocation is failed or not.
 *
 * Parameters:
 *      num_units	The number of memory units should be allocated
 *      unit_size   	The number of bytes per each memory unit
...
{
	num_units = (num_units) ? (num_units) : 1;
	void *ptr = palloc((num_units) * (unit_size));
	dt_check_error(ptr, "memory allocation failure");
	...
	)
{
	...
	uint32 *bitmap 	= (uint32*)dt_palloc0(n_bitmap,sizeof(uint32));
...
/*
 * Cast a text to c-style string.
 *
 * Parameters:
 *      t     The pointer points to the text
 * Return:
 *      A c-style string
 */
static
...
{
    dt_check_error(t, "the input parameter must not be null");

	char *s 	= NULL;
	size_t len	= 0;
	len			= VARSIZE(t) - VARHDRSZ;
	s 			= dt_palloc0(len + 1, 1);
...
/*
 * Cast a c-style string to PG/GP text. An optional len can be supplied
 * to specify how many characters in the original string should be
 * converted into the result. If len is negative, then the whole
 * original string will be converted.
 *
 * Parameters:
 *      s     The pointer points to the string
 *      len	  The length of the substring to be converted to the text
 * Return:
 *      A string represented by the text structure
 */
static
text*
dt_string_to_text
...
{
    dt_check_error (s, "the input parameter must not be null");

    if (len < 0)
    	len = strlen(s);

	text	   *result = (text *) dt_palloc(len + VARHDRSZ, 1);
	...
	)
{
	...
	char	   *fmt 		= dt_text_to_string(PG_GETARG_TEXT_PP(0));
	...
	if (nitems == 0)
		PG_RETURN_TEXT_P(dt_string_to_text(fmt, -1));

	int *position 	= (int*)dt_palloc0(nitems, sizeof(int));
	...
	PG_RETURN_TEXT_P(dt_string_to_text(buf.data, buf.len));
}
