...
CREATE OR REPLACE FUNCTION svec_out(svec)
    RETURNS cstring
    AS 'gp_svec.so'
    LANGUAGE C IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION svec_recv(internal)
    RETURNS svec
    AS 'gp_svec.so'
    LANGUAGE C IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION svec_send(svec)
    RETURNS bytea
    AS 'gp_svec.so'
    LANGUAGE C IMMUTABLE STRICT;
...
DROP CAST IF EXISTS (int2 AS svec) ;
DROP CAST IF EXISTS (integer AS svec) ;
DROP CAST IF EXISTS (bigint AS svec) ;
DROP CAST IF EXISTS (float4 AS svec) ;
DROP CAST IF EXISTS (float8 AS svec) ;
DROP CAST IF EXISTS (numeric AS svec) ;
...
DROP CAST IF EXISTS (svec AS float8[]) ;
DROP CAST IF EXISTS (float8[] AS svec) ;
...
DROP OPERATOR IF EXISTS < (svec, svec) CASCADE ;
DROP OPERATOR IF EXISTS <= (svec, svec) CASCADE ;
DROP OPERATOR IF EXISTS <> (svec, svec) ;
DROP OPERATOR IF EXISTS == (svec, svec) CASCADE ;
DROP OPERATOR IF EXISTS > (svec, svec) CASCADE ;
DROP OPERATOR IF EXISTS >= (svec, svec) CASCADE ;
DROP OPERATOR IF EXISTS *|| (int4, svec) ;
